...
import { device, merchantProfile, scanEvent } from '@/db/schema';
...
async function resolveProfileId(userId: string): Promise<number> {
  const row = await getDb().query.merchantProfile.findFirst({
    where: eq(merchantProfile.userId, userId),
  });
  if (!row) throw new AppError(403, 'NO_PROFILE', 'Merchant profile not set up');
  return row.id;
}
...
export async function overview(userId: string): Promise<AnalyticsOverview> {
  ...
  const owned = await db.query.device.findMany({
    where: eq(device.ownerId, profileId),
...
export async function breakdown(
  ...
  const owned = await db.query.device.findFirst({
    where: and(eq(device.id, deviceId), eq(device.ownerId, profileId)),
  });
