...
Sidebar and endpoint access are resolved from the seeded `permission` table
(`listNavForRole(role)` / `can(role, path)`, FR-037). Analytics renders inside the
Dashboard (owner role), not as a standalone route.
...
tables (via `npx @better-auth/cli migrate`). A project-owned `permission` table stores
endpoints/nav rows (`path`, `label`, `icon`, `is_menu`, allowed roles) and is seeded by
default (FR-036/037).
...
and the project-owned `permission` table.
No constitution violation is introduced: RBAC enforcement remains in
`domains/*/server/permissions.ts`, now backed by seeded `permission` rows queried via
`listNavForRole(role)`/`can(role, path)` (VII); claim-code secrets are stored hashed and
rotated on reset (VII); device FKs are DB-enforced; no business logic moved into
`app/` (I); the `(dashboard)` route group with root paths and permission-driven nav is
pure routing/composition, and each page prefetches data via TanStack Query (II); the
accountless setup and scan routes are server components with `loading.tsx`/Suspense
skeletons (II/IV); framer-motion is a client-only animation for auth screens with no
data fetching, so it complies with II and needs no loading-state exemption. Gate status
after Phase 1: all PASS.
...
│   ├── layout.tsx                  # auth guard + sidebar shell + next-themes; nav from permission table
│   ├── loading.tsx
│   ├── error.tsx
│   ├── dashboard/page.tsx          # /dashboard — role-aware; analytics section for owner/reseller
│   ├── devices/
...
├── relations.ts
├── index.ts
└── seed.ts                         # ADMIN, reseller org + owner, member, devices, permission rows
drizzle/                            # drizzle-kit generated migrations (sql + meta)
domains/
├── auth/                           # Better Auth + organization plugin config, RBAC
│   └── server/permissions.ts       # listNavForRole(role) + can(role, path) from permission table
...
role groups (FR-036).** Access and nav are driven by the seeded `permission` table:
`(dashboard)/layout.tsx` guards each request and renders the sidebar from
`listNavForRole(role)`; proxy.ts/`layout` checks `can(role, path)` for denied routes.
Route groups are exactly four: `(auth)`, `(dashboard)`, `(landing-page)`, `(redirect)`.
